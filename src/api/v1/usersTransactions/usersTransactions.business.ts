...
export default class UsersTransactionsBusiness {
  async index(req: Request, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ message: 'Não foi possível buscar transações ' });
    }
  }
  ...
  async create(req: Request, res: Response) {
    try {
      const repo = getCustomRepository(UsersTransactionsRepository);
      const repoTypeTransaction = getCustomRepository(
        TypeTransactionsRepository
      );

      const { name, value, typeTransaction } = req.body;

      const foundTypeTransaction = await repoTypeTransaction.getTypeTransaction(
        typeTransaction
      );
      ...
      if (!foundTypeTransaction) {
        return res
          .status(404)
          .json({ error: 'Este tipo de transação não existe' });
      }
        ...
        if (typeTransaction == 2) {
          ...
          if (value > balance) {
            return res
              .status(401)
              .json({
                erro: `Esta despesa é maior que o seu saldo! Saldo disponível: R$ ${balance},00`,
              });
          }
        }
      ...
      return res
        .status(500)
        .json({ message: 'Não foi possível criar transação ' });
    }
  }
}
