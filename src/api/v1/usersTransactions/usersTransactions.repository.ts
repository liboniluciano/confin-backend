...
export default class UsersTransactionsRepository extends Repository<
  UsersTransactions
> {
  public async createTransaction(
    transaction: UsersTransactions
  ): Promise<UsersTransactions> {
    return await this.save(transaction);
  }

  public async getIncomeOutome(
    typeTransaction: number,
    userId: number
  ): Promise<UsersTransactions> {
    const transaction = this.createQueryBuilder('ut')
      .select('SUM(ut.value)')
      .where('ut.typeTransaction = :idTransaction', {
        idTransaction: typeTransaction,
      })
